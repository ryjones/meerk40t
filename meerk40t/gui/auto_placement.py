"""
This module deals with the automatic placement of patterns by analyzing
a camera image and autodetecting edges.
This functionality is inspired by a feature within XCS called SmartFill
As this implementation does only part of it we call it DumbFill, and as 
the advertisement videos for the real thing show people just dumping 
things on the laserbed - well, I couldn't resist the pun.
The implementation relies on the contour detection login within OpenCV:
...
class DumpFill(MWindow):
    """
    External interface to DumpFill
    """
    def __init__(self, *args, **kwds):
        super().__init__(551, 234, submenu="Operations", *args, **kwds)
        self.panel = DumpFillPanel(self, wx.ID_ANY, context=self.context)
        self.sizer.Add(self.panel, 1, wx.EXPAND, 0)
        _icon = wx.NullIcon
        # _icon.CopyFromBitmap(icons8_computer_support.GetBitmap())
        self.SetIcon(_icon)
        self.SetTitle(_("Auto-place Template"))
        self.restore_aspect()
    ...
    @staticmethod
    def submenu():
        return "Laser-Tools", "Auto-Template"
